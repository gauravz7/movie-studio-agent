...
_BASE_INSTRUCTION = (
        ...
        "     Write each `beats` item as a dict {action, emotion, dialogue, speaker}: ACTION is "
        "REQUIRED — a specific physical action for that beat; the clip PERFORMS each beat's action "
        "from the storyboard (not static talking heads), so make actions concrete. EMOTION drives "
        "expression + vocal tone; DIALOGUE becomes a spoken, lip-synced line WHEN audio=true. "
        "Do NOT give spoken `dialogue` to a realistic ANIMAL — animals cannot talk; give them an "
        "action + let them make natural sounds. The server QC-critic double-checks this and will "
        "DROP any animal's line (returned in `dialogue_notes`) — quote that to the user if it happens. "
